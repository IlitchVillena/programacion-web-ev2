async function listarRegiones() {
    ...
    const method = 'GET';
    /* LLAMADO A API REST LISTAR REGIONES DIVISIÓN POLITICO ADMINISTRATIVA  */
    ...
    const resp = await fetch(url, {
        ...
        selectOneRegion = `<select id="lista-regiones" 
                          class="form-control border-primary mb-3" aria-label="Select region" onclick="listarComunas()">`;
        ...
        labelRegion = `<label for="lista-regiones"</label>`;
...
async function listarComunas(region_id) {
    ...
    const resp = await fetch(url, {
        ...
        /* SECCIÓN PARA LEER data.json */
        selectOneComuna = `<select id="lista-comunas" 
                          class="form-control border-primary mb-3" aria-label="Select comunas">`;
        ...
        labelComuna = `<label for="lista-comunas"</label>`;
